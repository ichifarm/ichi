...
contract ICHIPOWAH {
  ...
  function totalSupply() public view returns (uint256) {
    ...
    IERC20 ICHI = IERC20(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    (uint256 lp_totalICHI, , ) = pair.getReserves();
    uint256 xICHI_totalICHI = ICHI.balanceOf(address(bar));
  ...
  function balanceOf(address owner) public view returns (uint256) {
    ...
    uint256 xICHI_balance = bar.balanceOf(owner);
    uint256 xICHI_total = bar.totalSupply();
    uint256 xICHI_totalICHI = ICHI.balanceOf(address(bar));
    
    // xICHI voting power is the users ICHI share in the bar
